...
var masterdata = {};
	...
	$("#propertyregsitration_from").validate({
		    errorPlacement: function(error, element) {
				$('#propertyregsitration_from div.stepy-error').append(error);
			}, rules: {
				//'city':	 'required',
				'address1':	'required',
				'filenumber': 'required',
				...
				'address1':			{ required:  'Address 1 field is requerid!' },
				'filenumber':			{ required:  'File Number field is requerid!' },
				'district':			{ required:  'District is required' },
				'state': { required:  'State is required' },
				'bldgtype': { required:  'Building Type is required' },
				'subzone': { required:  'Subzone is required' },
				...
				'postcode': {
	                minlength: 'Post Code must be at least 5 characters long',
	                maxlength: 'Post Code must be at least 6 characters long',
	...
	$('#stepy_form').validate({
					...
					/*	'masterhot' : masterhot.validateCells((valid) => {
  		if (valid) {
  			'';
  		} else {
  			'required';
...
function fnFormatDetails ( oTable, nTr )
{
    ...
    sOut += '<tr><td>Update By / Update At:</td><td>'+aData[11]+'</td></tr>';
    sOut += '<tr><td>Create By / Create At:</td><td>'+aData[10]+'</td></tr>';
    sOut += '<tr><td>Notice Count:</td><td>'+aData[12]+'</td></tr>';
    sOut += '<tr><td>Objection Count:</td><td>'+aData[13]+'</td></tr>';
    sOut += '<tr><td>Decision Count:</td><td>'+aData[14]+'</td></tr>';
    sOut += '</table>';
...
$(function() {
    ...
    $('.termtbl_details thead tr').each( function () {
        this.insertBefore( nCloneTh, this.childNodes[0] );
    } );
     
    $('.termtbl_details tbody tr').each( function () {
        this.insertBefore(  nCloneTd.cloneNode( true ), this.childNodes[0] );
    } );
     ...
     * Initialse DataTables, with no sorting on the 'details' column
     */
    var oTable = $('.termtbl_details').dataTable( {
        "aoColumnDefs": [
            { "bSortable": false, "aTargets": [ 0 ] }
        ],
        "aaSorting": [[1, 'asc']],
     ...
     * rather it is done here
     */
    $('.termtbl_details tbody td img').live('click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oTable.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
        ...
        else
        {
            /* Open this row */
            this.src = "images/details_close.png";
            oTable.fnOpen( nTr, fnTermFormatDetails(oTable, nTr), 'details' );
        }
    } );
} );
	

	function fnTermFormatDetails ( oTable, nTr )
{
    var aData = oTable.fnGetData( nTr );
    var sOut = '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
    //sOut += '<tr><td>Tittle:</td><td>'+aData[2]+' '+aData[4]+'</td></tr>';
    sOut += '<tr><td>Create By:</td><td>'+aData[7]+'</td><td>Create At:</td><td>'+aData[8]+'</td>'+
...
$(function() {
    ...
    $('.plantbl_details thead tr').each( function () {
        this.insertBefore( nCloneTh, this.childNodes[0] );
    } );
     
    $('.plantbl_details tbody tr').each( function () {
        this.insertBefore(  nCloneTd.cloneNode( true ), this.childNodes[0] );
    } );
     ...
     * Initialse DataTables, with no sorting on the 'details' column
     */
    var oTable = $('.plantbl_details').dataTable( {
        "aoColumnDefs": [
            { "bSortable": false, "aTargets": [ 0 ] }
        ],
        "aaSorting": [[1, 'asc']],
     ...
     * rather it is done here
     */
    $('.plantbl_details tbody td img').live('click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oTable.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
        ...
        else
        {
            /* Open this row */
            this.src = "images/details_close.png";
            oTable.fnOpen( nTr, fnPlanFormatDetails(oTable, nTr), 'details' );
        }
    } );
} );
	

	function fnPlanFormatDetails ( oTable, nTr )
{
    var aData = oTable.fnGetData( nTr );
    var sOut = '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
    //sOut += '<tr><td>Tittle:</td><td>'+aData[2]+' '+aData[4]+'</td></tr>';
...
$(function() {
    ...
    $('.termtbl_details1 thead tr').each( function () {
        this.insertBefore( nCloneTh, this.childNodes[0] );
    } );
     
    $('.termtbl_details1 tbody tr').each( function () {
        this.insertBefore(  nCloneTd.cloneNode( true ), this.childNodes[0] );
    } );
     ...
     * Initialse DataTables, with no sorting on the 'details' column
     */
    var oTable = $('.termtbl_details1').dataTable( {
        "aoColumnDefs": [
            { "bSortable": false, "aTargets": [ 0 ] }
        ],
        "aaSorting": [[1, 'asc']],
     ...
     * rather it is done here
     */
    $('.termtbl_details1 tbody td img').live('click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oTable.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
